...
def display_table(
    ...
    if isinstance(astro_table, list) and all(
        isinstance(row, list) for row in astro_table
    ):
        if not astro_table:
            console.print(
                Padding(
        ...
        try:
            ...
            rich_table = RichTable(
                title=title, show_lines=True, header_style="bold magenta", expand=False
            )
            for h in headers:
                rich_table.add_column(
                    h,
                    overflow="fold" if max_col_width else "ellipsis",
    ...
    if show_all_columns:
        ...
        if "name" in displayed_columns and "description" in displayed_columns:
            console.print(
                f"[yellow]{builtins._('Terminal is narrow. Displaying only mission names for brevity. Use --show-all-cols to see descriptions.')}[/yellow]"
            )
            displayed_columns = ["name"]
        else:  # If not mission list, or no description, still limit to 10 if not show_all_columns
            console.print(
                f"[cyan]Table has {len(astro_table.colnames)} columns. Displaying first 10. Use --show-all-cols to see all.[/cyan]"
            )
    ...
    if show_ellipsis:
        ...
        if max_rows == -1:  # Hard limit was applied
            console.print(
                f"... and {len(astro_table) - num_rows_to_display} more rows. To display all, use --max-rows-display {len(astro_table)} or a sufficiently large number."
            )
        else:  # User-specified limit was applied
            console.print(
                f"... and {len(astro_table) - max_rows} more rows. Use --max-rows-display -1 to display all rows."
            )
    console.print(Padding(f"Total rows: {len(astro_table)}", (0, 2)))
...
common_output_options = {
    "output_file": typer.Option(
        ...
        "-o",
        help=builtins._(
            "Path to save the output table (e.g., data.csv, results.ecsv, table.fits). Format inferred from extension."
        ),
    ),
    "output_format": typer.Option(
        ...
        "-f",
        help=builtins._(
            "Astropy table format for saving (e.g., 'csv', 'ecsv', 'fits', 'votable'). Overrides inference from filename extension."
        ),
    ),
}
...
def save_table_to_file(
    ...
    if not file_format:
        ...
        if ext:
            ...
            console.print(
                f"[yellow]No file extension or format specified, saving as '{filename}' (ECSV format).[/yellow]"
            )
    ...
    try:
        ...
        elif file_format not in AstropyTable.write.formats and file_format not in [
            "pickle",
            "pkl",
        ]:
            console.print(
                f"[yellow]Available astropy table write formats: {list(AstropyTable.write.formats.keys())}[/yellow]"
            )
...
def global_keyboard_interrupt_handler(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            console = Console()
            console.print(
                f"[bold yellow]{_('User interrupted the query. Exiting safely.')}[bold yellow]"
            )
            import os
