...
def setup_subcommands():
    ...
    app.add_typer(
        vizier_cli.get_app(),
        name="vizier",
        help=builtins._("Query the VizieR astronomical catalog database. (viz)"),
    )
    app.add_typer(vizier_cli.get_app(), name="viz")  # Alias for vizier
    app.add_typer(
        simbad_cli.get_app(),
        name="simbad",
        help=builtins._("Query the SIMBAD astronomical database. (sim)"),
    )
    app.add_typer(simbad_cli.get_app(), name="sim")  # Alias for simbad
    app.add_typer(jpl_cli.get_app(), name="jpl")  # Updated for jpl
    app.add_typer(
        exoplanet_cli.get_app(),
        name="exoplanet",
        help=builtins._("Query the NASA Exoplanet Archive. (exo)"),
    )
    app.add_typer(exoplanet_cli.get_app(), name="exo")  # Alias for exoplanet
    app.add_typer(
        heasarc_cli.get_app(),
        name="heasarc",
        help=builtins._("Query the HEASARC database. (hea)"),
    )
    ...
    if ctx.invoked_subcommand is None and not any(
        ...
        if not ping and not field:
            ...
            full_help_text = re.sub(
                r"Please note that the Gaia ESA Archive has been rolled back to version 3\.7\..*?release-notes\)?\n?",
                "",
                full_help_text,
                flags=re.DOTALL,
            )
