...
def get_app():
    ...
    @app.command(
        name="object", help=builtins._("Query ALMA for observations of an object.")
    )
    @global_keyboard_interrupt_handler
    def query_object(
        ctx: typer.Context,
        object_name: str = typer.Argument(
            ...,
            help=builtins._(
                "Name of the astronomical object, e.g., 'Orion KL', 'Sgr B2', 'NGC 253'."
            ),
        ),
        public_data: bool = typer.Option(
            True, help=builtins._("Query only public data.")
        ),
        science_data: bool = typer.Option(
            True, help=builtins._("Query only science data.")
        ),
        payload: Optional[List[str]] = typer.Option(
            None,
            "--payload-field",
            help=builtins._(
                "Specify payload fields to query (e.g., 'band_list', 'target_name')."
            ),
        ),
            ...
            if payload:
                for item in payload:
                    if "=" in item:
                        ...
                        console.print(
                            _(
                                "[yellow]Payload item '{item}' is not a key=value pair. Ignoring.[/yellow]"
                            ).format(item=item)
                        )

            result_table: Optional[AstropyTable] = alma.query(
                payload=query_payload, public=public_data, science=science_data
            )

            if result_table and len(result_table) > 0:
                console.print(
                    _(
                        "[green]Found {count} match(es) for '{object_name}'.[/green]"
    ...
    @app.command(
        name="region", help=builtins._("Query ALMA for observations in a sky region.")
    )
    @global_keyboard_interrupt_handler
    def query_region(
        ctx: typer.Context,
        coordinates: str = typer.Argument(
            ..., help=builtins._("Coordinates (e.g., '10.68h +41.26d', '150.0 2.0').")
        ),
        ...
        try:
            ...
            console.print(
                f"[cyan]{_('Querying ALMA for region: {coordinates} with radius {radius}').format(coordinates=coordinates, radius=radius)}[/cyan]"
            )
            ...
            result_table: Optional[AstropyTable] = alma.query_region(
                coord, radius=rad, public=public_data, science=science_data
            )

            if result_table and len(result_table) > 0:
                console.print(
                    _("[green]Found {count} match(es) in the region.[/green]").format(
                ...
                console.print(
                    _("[yellow]No information found for the specified region.[/yellow]")
                )
        except Exception as e:
