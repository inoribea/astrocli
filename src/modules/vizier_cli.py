...
def get_app():
    ...
    def vizier_callback(
        ...
        ):  # Use ctx.args for subcommand arguments
    ...
    def parse_angle_str_to_quantity(
        ctx: typer.Context, angle_str: Optional[str]
    ) -> Optional[u.Quantity]:
        ...
        try:
            ...
            console.print(
                _(
                    "[yellow]Hint: Use format like '5arcmin', '0.5deg', '10arcsec'.[/yellow]"
                )
            )
    ...
    def parse_coordinates(ctx: typer.Context, coords_str: str) -> SkyCoord:
        try:
            if "," in coords_str and (
                "h" in coords_str or "d" in coords_str or ":" in coords_str
            ):
            ...
            try:
                ...
                console.print(
                    _(
                        "[yellow]Hint: Try 'M31', '10.68h +41.26d', or '160.32 41.45'.[/yellow]"
                    )
                )
                raise typer.Exit(code=1)

    def parse_constraints_list(
        ctx: typer.Context, constraints_list: Optional[List[str]]
    ) -> dict:
        parsed_constraints = {}
        if constraints_list:
            for item in constraints_list:
                if "=" not in item:
                    console.print(
                        _(
                            "[bold red]Invalid constraint format: '{item}'. Expected 'column=condition'.[/bold red]"
                        ).format(item=item)
                    )
                    raise typer.Exit(code=1)
    ...
    @app.command(
        name="find-catalogs",
        help=builtins._(
            "Find VizieR catalogs based on keywords, UCDs, or source names."
        ),
    )
    def find_catalogs(
        ctx: typer.Context,
        keywords: Optional[List[str]] = typer.Option(
            ...
            "--keyword",
            "-k",
            help=builtins._("Keyword(s) to search for in catalog descriptions."),
        ),
        ucd: Optional[str] = typer.Option(
            None,
            help=builtins._("UCD (Unified Content Descriptor) to filter catalogs."),
        ),
        source_name: Optional[str] = typer.Option(
            None,
            "--source",
            help=builtins._("Source name or pattern (e.g., 'Gaia DR3', '2MASS')."),
        ),
        max_catalogs: int = typer.Option(
            20, help=builtins._("Maximum number of catalogs to list.")
        ),
        ...
        vizier_server: str = typer.Option(
            "vizier_cds",
            help=builtins._("VizieR server to use. Choices: {server_list}").format(
                server_list=list(VIZIER_SERVERS.keys())
            ),
        ...
        vizier_conf.server = VIZIER_SERVERS.get(
            vizier_server.lower(), vizier_conf.server
        )
        debug(
            _("Using VizieR server: {server_url}").format(server_url=vizier_conf.server)
        )
        ...
        if not query_params:
            console.print(
                _(
                    "[yellow]Please provide at least one search criterion (keyword, ucd, or source name).[/yellow]"
                )
            )
            console.print(
                _(
                    "Example: `aqc vizier find-catalogs --keyword photometry --keyword M31`"
                )
            )
            raise typer.Exit(code=1)
        ...
        try:
            ...
            if result_tables and len(result_tables) > 0 and 0 in result_tables.keys():
                ...
                console.print(
                    _("[yellow]No catalogs found matching your criteria.[/yellow]")
                )
                return
    ...
    def list_catalogs(
        ...
        console.print(
            _(
                "[yellow]Note: If this returns no catalogs, the VizieR service might not support listing all catalogs directly, or there might be a network issue. Consider using 'find-catalogs' with specific keywords instead.[/yellow]"
            )
        )
        vizier_conf.server = VIZIER_SERVERS.get(
            vizier_server.lower(), vizier_conf.server
        ...
        try:
            ...
            if all_catalogs:
                ...
                console.print(
                    _(
                        "[yellow]Example: aqc vizier find-catalogs --keyword photometry --keyword galaxy[/yellow]"
                    )
                )
                return
    ...
    @app.command(
        name="object",
        help=builtins._(
            "Query catalogs around an object name or specific coordinates."
        ),
    )
    @global_keyboard_interrupt_handler
    def query_object(
        ...
        target: str = typer.Argument(
            ...,
            help=builtins._(
                "Object name (e.g., 'M31') or coordinates (e.g., '10.68h +41.26d' or '160.32 41.45')."
            ),
        ),
        radius: str = typer.Argument(
            ...,
            help=builtins._(
                "Search radius (e.g., '5arcmin', '0.1deg'). Can be specified as positional argument or with -r/--radius."
            ),
        ...
        catalogs: Optional[List[str]] = typer.Option(
            ...
            "-c",
            help=builtins._(
                "VizieR catalog identifier(s) (e.g., 'I/261/gaiadr3', 'J/ApJ/710/1776'). Can be specified multiple times. Default: 'I/261/gaiadr3'"
            ),
        ),
        columns: Optional[List[str]] = typer.Option(
            ...
            "--col",
            help=builtins._(
                "Specific columns to retrieve (e.g., 'RAJ2000', 'DEJ2000', 'pmRA'). Use 'all' for all columns. Can be specified multiple times."
            ),
        ),
        column_filters: Optional[List[str]] = typer.Option(
            None,
            "--filter",
            help=builtins._(
                "Column filters (e.g., 'Imag<15', 'B-V>0.5'). Can be specified multiple times. Format: 'column_name<operator>value'."
            ),
        ),
        row_limit: int = typer.Option(
            vizier_conf.row_limit,
            help=builtins._("Maximum number of rows to return per catalog."),
        ),
        max_rows_display: int = typer.Option(
            20,
            help=builtins._(
                "Maximum number of rows to display per table. Use -1 for all rows."
            ),
        ),
        show_all_columns: bool = typer.Option(
            False,
        ...
        console.print(
            _(
                "[cyan]Querying VizieR for object '{target_name}' in catalog(s): {catalog_list}...[/cyan]"
            ).format(target_name=target, catalog_list=", ".join(catalogs_to_query))
        )
        vizier_conf.server = VIZIER_SERVERS.get(
            vizier_server.lower(), vizier_conf.server
        )
        ...
        debug(
            _("Using VizieR server: {server_url}, Row limit: {limit}").format(
                server_url=vizier_conf.server, limit=row_limit
            )
        )
        ...
        viz = Vizier(
            ...
            column_filters=processed_column_filters,
            row_limit=row_limit,
        )
        ...
        try:
            ...
            if not result_tables:
                console.print(
                    _(
                        "[yellow]No results returned from VizieR for this query.[/yellow]"
                    )
                )
                return
            ...
            for table_name in result_tables.keys():
                ...
                if table_data is not None and len(table_data) > 0:
                    display_table(
                        ctx,
                        table_data,
                        title=_("Results from {catalog_name} for {target_name}").format(
                            catalog_name=table_name, target_name=target
                        ),
                    ...
                    console.print(
                        _(
                            "[yellow]No data found in catalog '{catalog_name}' for the given criteria.[/yellow]"
    ...
    @app.command(
        name="region",
        help=builtins._("Query catalogs within a sky region (cone or box)."),
    )
    @global_keyboard_interrupt_handler
    def query_region(
        ctx: typer.Context,
        coordinates: str = typer.Argument(
            ...,
            help=builtins._(
                "Central coordinates for the region (e.g., '10.68h +41.26d' or '160.32 41.45')."
            ),
        ),
        radius: Optional[str] = typer.Argument(
            None,
            help=builtins._(
                "Cone search radius (e.g., '5arcmin', '0.1deg'). Use if not specifying width/height. Can be specified as positional argument or with -r/--radius."
            ),
            rich_help_panel="Arguments",
            show_default=False,
        ),
        width: Optional[str] = typer.Option(
            ...
            "--width",
            help=builtins._(
                "Width of a box region (e.g., '10arcmin', '0.5deg'). Requires --height."
            ),
        ),
        height: Optional[str] = typer.Option(
            None,
            "--height",
            help=builtins._(
                "Height of a box region (e.g., '10arcmin', '0.5deg'). Requires --width."
            ),
        ),
        catalogs: Optional[List[str]] = typer.Option(
            None,
        ...
        columns: Optional[List[str]] = typer.Option(
            ...
            "--col",
            help=builtins._(
                "Specific columns to retrieve. Use 'all' for all columns. Can be specified multiple times."
            ),
        ),
        column_filters: Optional[List[str]] = typer.Option(
            None,
            "--filter",
            help=builtins._(
                "Column filters (e.g., 'Imag<15'). Can be specified multiple times."
            ),
        ),
        row_limit: int = typer.Option(
            vizier_conf.row_limit,
        ...
        console.print(
            _(
                f"[cyan]Querying VizieR region around '{coordinates}' in catalog(s): {', '.join(catalogs_to_query)}...[/cyan]"
            )
        )
        vizier_conf.server = VIZIER_SERVERS.get(
            vizier_server.lower(), vizier_conf.server
        ...
        if rad_quantity is not None and (
            width_quantity is not None or height_quantity is not None
        ):
            console.print(
                _(
                    f"[bold red]Error: Specify either --radius (for cone search) OR (--width and --height) (for box search), not both.[/bold red]"
                )
            )
            raise typer.Exit(code=1)
        if (width_quantity is not None and height_quantity is None) or (
            width_quantity is None and height_quantity is not None
        ):
            console.print(
                _(
                    f"[bold red]Error: For a box search, both --width and --height must be specified.[/bold red]"
                )
            )
            raise typer.Exit(code=1)
        if rad_quantity is None and (
            width_quantity is None and height_quantity is None
        ):
            console.print(
                _(
                    f"[bold red]Error: You must specify search dimensions: either --radius OR (--width and --height).[/bold red]"
                )
            )
            raise typer.Exit(code=1)
            ...
            for idx, table_name in enumerate(result_tables.keys()):
                ...
                if table_data is not None and len(table_data) > 0:
                    display_table(
                        ...
                        table_data,
                        title=_(
                            f"Results from {table_name} for region around {coordinates}"
                        ),
                        max_rows=max_rows_display,
                        show_all_columns=show_all_columns,
                    )
    ...
    @app.command(
        name="constraints",
        help=builtins._(
            "Query catalogs based on specific column constraints or keywords."
        ),
    )
    @global_keyboard_interrupt_handler
    def query_constraints(
        ...
        catalogs: List[str] = typer.Option(
            ...
            "-c",
            help=builtins._(
                "VizieR catalog identifier(s). Can be specified multiple times."
            ),
        ),
        constraints: Optional[List[str]] = typer.Option(
            ...
            "--constraint",
            help=builtins._(
                "Constraints on column values (e.g., 'Vmag=<10', 'B-V=0.5..1.0'). Can be specified multiple times. Format: 'column_name=condition'."
            ),
        ),
        keywords: Optional[List[str]] = typer.Option(
            None,
            ...
            "-k",
            help=builtins._(
                "Keywords to filter results within the catalog (different from finding catalogs)."
            ),
        ),
        columns: Optional[List[str]] = typer.Option(
            None,
        ...
        console.print(
            _(
                f"[cyan]Querying VizieR with constraints in catalog(s): {', '.join(catalogs)}...[/cyan]"
            )
        )
        vizier_conf.server = VIZIER_SERVERS.get(
            vizier_server.lower(), vizier_conf.server
        ...
        if not parsed_constraints and not keywords:
            console.print(
                _(
                    "[yellow]Please provide at least --constraint(s) or --keyword(s) for this query type.[/yellow]"
                )
            )
            raise typer.Exit(code=1)
            ...
            for table_name in result_tables.keys():
                ...
                if table_data is not None and len(table_data) > 0:
                    display_table(
                        ctx,
                        table_data,
                        title=_(f"Constraint Query Results from {table_name}"),
                        max_rows=max_rows_display,
                        show_all_columns=show_all_columns,
                    )
                else:
