...
def get_app():
    ...
    def parse_range(range_str: str, unit_type: str) -> Tuple[u.Quantity, u.Quantity]:
        ...
        if len(parts) != 2:
            if unit_type == "wavelength":
                console.print(
                    _(
                        "[red]Error: Wavelength range '{range_str}' must be in 'min-max' format (e.g., '100um-200um').[/red]"
                    ).format(range_str=em(str(range_str)))
                )
            else:
                console.print(
                    _(
                        "[red]Error: Frequency range '{range_str}' must be in 'min-max' format (e.g., '100GHz-110GHz').[/red]"
                    ).format(range_str=em(str(range_str)))
                )
        ...
        try:
            ...
            if unit_type == "wavelength":
                if not (
                    min_val.unit.is_equivalent(u.m) and max_val.unit.is_equivalent(u.m)
                ):
                    console.print(
                        _(
                            "[red]Error: Wavelengths must have units of length (e.g., um, nm, mm).[/red]"
                        )
                    )
                    raise typer.Exit(code=1)
            else:
                if not (
                    min_val.unit.is_equivalent(u.Hz)
                    and max_val.unit.is_equivalent(u.Hz)
                ):
                    console.print(
                        _(
                            "[red]Error: Frequencies must have units of frequency (e.g., GHz, MHz).[/red]"
                        )
                    )
                    raise typer.Exit(code=1)
            ...
            console.print(
                _(
                    "[red]Error parsing {unit_type} range '{range_str}': {error_message}[/red]"
                ).format(
                    unit_type=unit_type,
                    range_str=em(str(range_str)),
                    error_message=em(str(e)),
                )
            )
            raise typer.Exit(code=1)
    ...
    @app.command(
        name="lines",
        help=builtins._(
            "Query spectral lines from Splatalogue by chemical name. Optionally filter by wavelength (-w) or frequency (-f) range."
        ),
    )
    @global_keyboard_interrupt_handler
    def query_lines(
        ...
        wavelength_range: Optional[str] = typer.Argument(
            None,
            help=builtins._(
                "Wavelength range (e.g., '100um-200um'). You can directly input this as the second argument without -w. Mutually exclusive with frequency_range."
            ),
        ),
        frequency_range: Optional[str] = typer.Argument(
            None,
            help=builtins._(
                "Frequency range (e.g., '100GHz-110GHz'). You can directly input this as the second argument without -f. Mutually exclusive with wavelength_range."
            ),
        ),
        energy_max: Optional[float] = typer.Option(
            None, help=builtins._("Maximum energy in K (E_upper).")
        ),
        energy_type: Optional[str] = typer.Option(
            "el", help=builtins._("Energy type ('el' or 'eu' for E_lower or E_upper).")
        ),
        line_strengths: Optional[str] = typer.Option(
            None,
            help=builtins._(
                "Line strength units (e.g., 'ls1', 'ls2', 'ls4', 'ls5' for CDMS/JPL or TopModel)."
            ),
        ),
        exclude: Optional[List[str]] = typer.Option(
            None, "--exclude", help=builtins._("Species to exclude (e.g., 'HDO').")
        ),
        ...
        output_format: Optional[str] = typer.Option(
            ...
            "--output-format",
            help=builtins._(
                "Astropy table format for saving (e.g. 'csv', 'ecsv', 'fits', 'votable'). Overrides file extension inference."
            ),
        ),
        ...
        if wavelength_range and frequency_range:
            console.print(
                _(
                    "[red]You cannot specify both -w/--wavelength-range and -f/--frequency-range.[/red]"
                )
            )
            raise typer.Exit(code=1)
        ...
        try:
            ...
            if wavelength_range:
                ...
                console.print(
                    "[cyan]Querying Splatalogue for '{chemical_name}' in wavelength range '{wavelength_range}' ({min_freq:.2e}Hz - {max_freq:.2e}Hz)...[/cyan]".format(
                        chemical_name=em(str(chemical_name)),
                        wavelength_range=em(str(wavelength_range)),
                        min_freq=min_freq.value,
                        max_freq=max_freq.value,
                    )
                )
                ...
                console.print(
                    _(
                        "[cyan]Querying Splatalogue for '{chemical_name}' in frequency range '{frequency_range}'...[/cyan]"
                    ).format(
                        chemical_name=em(str(chemical_name)),
                        frequency_range=em(str(frequency_range)),
                    )
                )
            else:
                console.print(
                    _(
                        "[red]Error: You must specify a wavelength or frequency range as the second argument (e.g., '100um-200um' or '100GHz-110GHz'). No -w or -f flag is needed; just provide the range directly.[/red]"
                    )
                )
                raise typer.Exit(code=1)
            ...
            if min_freq is not None and max_freq is not None:
                result_table: Optional[AstropyTable] = Splatalogue.query_lines(
                    min_frequency=min_freq, max_frequency=max_freq, **kwargs
                )
            else:
                result_table: Optional[AstropyTable] = Splatalogue.query_lines(**kwargs)

            if result_table and len(result_table) > 0:
                console.print(
                    _("[green]Found {count} spectral line(s).[/green]").format(
                ...
                if output_file:
                    save_table_to_file(
                        ...
                        output_file,
                        output_format,
                        _("Splatalogue line query"),
                    )
            else:
                console.print(
                    _(
                        "[yellow]No spectral lines found for the given criteria.[/yellow]"
                    )
                )
        except Exception as e:
            if isinstance(
                e, ValueError
            ) and "No matching chemical species found" in str(e):
                console.print(
                    _(
                        "[yellow]No matching chemical species found. Please check your chemical_name pattern and try removing wildcards or using a more specific name.[/yellow]"
                    )
                )
    ...
    @app.command(
        name="species-table",
        help=builtins._("Get the table of NRAO recommended species."),
    )
    @global_keyboard_interrupt_handler
    def get_species_table(
        ...
        console.print(
            _(
                "[cyan]Fetching NRAO recommended species table from Splatalogue...[/cyan]"
            )
        )
        try:
            ...
            if species_table and len(species_table) > 0:
                display_table(
                    ctx,
                    species_table,
                    title=_("Splatalogue NRAO Recommended Species"),
                    max_rows=max_rows_display,
                ...
                if output_file:
                    save_table_to_file(
                        ...
                        output_file,
                        output_format,
                        _("Splatalogue species table"),
                    )
            else:
                console.print(
                    _(
                        "[yellow]Could not retrieve species table or it is empty.[/yellow]"
                    )
                )
        except Exception as e:
