...
def get_app():
    ...
    @app.command(
        name="object",
        help=builtins._(
            "Query Gaia main source for a given object name or coordinates."
        ),
    )
    ...
    def query_object(
        ...
        target: str = typer.Argument(
            ...,
            help=builtins._(
                "Object name or coordinates (e.g., 'M31', '10.68h +41.26d')."
            ),
        ),
        radius: str = typer.Option(
            "5arcsec",
            help=builtins._(
                "Search radius for matching Gaia source (e.g., '5arcsec', '0.001deg')."
            ),
        ),
        ...
        columns: Optional[List[str]] = typer.Option(
            ...
            "--col",
            help=builtins._(
                "Columns to retrieve (e.g., 'source_id', 'ra', 'dec', 'parallax')."
            ),
        ),
        ...
        try:
            ...
            """
            console.print(
                builtins._("[cyan]Querying Gaia for object: {target}...[/cyan]").format(
                    target=target
                )
            )  # Use builtins._
            ...
            debug(
                f"Results retrieved. Table length: {len(result_table) if result_table is not None else 0}"
            )
            ...
            if result_table is not None and len(result_table) > 0:
                ...
                console.print(
                    _(
                        "[yellow]No Gaia source found for '{target}' in the given radius.[/yellow]"
    ...
    @app.command(
        name="cone-search",
        help=builtins._("Perform a cone search around a coordinate."),
    )
    @global_keyboard_interrupt_handler
    def cone_search(
        ctx: typer.Context,
        ...
        radius: str = typer.Option(
            "10arcsec", help=builtins._("Search radius (e.g., '5arcmin', '0.1deg').")
        ),
        table_name: str = typer.Option(
            GAIA_TABLES["main_source"],
            help=builtins._(
                "Gaia table to query. Common choices: {choices} or specify full table name."
            ).format(choices=list(GAIA_TABLES.keys())),
            autocompletion=lambda: list(GAIA_TABLES.keys()),
        ),
        columns: Optional[List[str]] = typer.Option(
            None,
            "--col",
            help=builtins._(
                "Specific columns to retrieve (e.g., 'source_id', 'ra', 'dec', 'pmra'). Default: all columns from the table for a small radius, or a default set for larger radii."
            ),
        ),
        row_limit: int = typer.Option(
            1000, help=builtins._("Maximum number of rows to return from the server.")
        ),
        output_file: Optional[str] = common_output_options["output_file"],
        output_format: Optional[str] = common_output_options["output_format"],
        max_rows_display: int = typer.Option(
        ...
        console.print(
            _(
                "[cyan]Performing Gaia cone search on '{table_name}' around '{target}' with radius {radius}...[/cyan]"
            ).format(table_name=resolved_table_name, target=target, radius=radius)
        )
        ...
        try:
            ...
            if result_table is not None and len(result_table) > 0:
                ...
                console.print(
                    _(
                        "[yellow]ADQL query returned no results or an empty table.[/yellow]"
                    )
                )
