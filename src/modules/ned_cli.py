...
def get_app():
    ...
    def query_object(
        ...
        object_name: str = typer.Argument(
            ..., help=builtins._("Name of the extragalactic object.")
        ),
        ...
        max_rows_display: int = typer.Option(
            1,
            help=builtins._(
                "Maximum number of objects to display (usually 1 for direct name)."
            ),
        ),
        ...
        console.print(
            _("[cyan]Querying NED for object: '{object_name}'...[/cyan]").format(
        ...
        try:
            ...
            if result_table and len(result_table) > 0:
                console.print(
                    _("[green]Found information for '{object_name}'.[/green]").format(
                        object_name=object_name
                    )
                )
                display_table(
                    ctx,
                    result_table,
                    title=_("NED Data for {object_name}").format(
                        object_name=object_name
                    ),
    ...
    @app.command(
        name="region", help=builtins._("Query NED for objects in a sky region.")
    )
    @global_keyboard_interrupt_handler
    def query_region(
        ctx: typer.Context,
        ...
        radius: str = typer.Argument(
            ..., help=builtins._("Search radius (e.g., '5arcmin', '0.1deg').")
        ),
        equinox: str = typer.Option(
            "J2000", help=builtins._("Equinox of coordinates (e.g., 'J2000', 'B1950').")
        ),
        output_file: Optional[str] = common_output_options["output_file"],
        output_format: Optional[str] = common_output_options["output_format"],
        max_rows_display: int = typer.Option(
        ...
        console.print(
            _(
                "[cyan]Querying NED for region: '{coordinates}' with radius '{radius}'...[/cyan]"
        ...
        try:
            ...
            result_table: Optional[AstropyTable] = Ned.query_region(
                coord, radius=rad_quantity, equinox=equinox
            )

            if result_table and len(result_table) > 0:
                console.print(
                    _("[green]Found {count} object(s) in the region.[/green]").format(
                ...
                console.print(
                    _("[yellow]No objects found in the specified region.[/yellow]")
                )
        except Exception as e:
    ...
    @app.command(
        name="images", help=builtins._("Get image metadata for an object from NED.")
    )
    @global_keyboard_interrupt_handler
    def get_images(
        ctx: typer.Context,
        ...
        max_rows_display: int = typer.Option(
            10,
            help=builtins._(
                "Maximum number of image entries to display. Use -1 for all rows."
            ),
        ),
        show_all_columns: bool = typer.Option(
            True,
        ...
        console.print(
            _(
                "[cyan]Fetching image list from NED for object: '{object_name}'...[/cyan]"
            ).format(object_name=object_name)
        )
        try:
            ...
            if images_table and len(images_table) > 0:
                console.print(
                    _(
                        "[green]Found {count} image entries for '{object_name}'.[/green]"
                    ).format(count=len(images_table), object_name=object_name)
                )
                display_table(
                    ctx,
                    images_table,
                    title=_("NED Image List for {object_name}").format(
                        object_name=object_name
                    ),
                    max_rows=max_rows_display,
                    show_all_columns=show_all_columns,
                ...
                if output_file:
                    save_table_to_file(
                        ...
                        output_file,
                        output_format,
                        _("NED image list query"),
                    )
            else:
                console.print(
                    _(
                        "[yellow]No image entries found for object '{object_name}'.[/yellow]"
                    ).format(object_name=object_name)
                )
        except Exception as e:
