...
def get_app():
    ...
    def query_gator(
        ...
        radius: Optional[str] = typer.Argument(
            None,
            help=builtins._(
                "Search radius (e.g., '10arcsec', '0.5deg'). Required if coordinates provided."
            ),
        ),
        catalog: Optional[str] = typer.Option(
            ...
            "-C",
            help=builtins._(
                "Explicitly specify the IRSA catalog name (e.g., 'allwise_p3as_psd'). Defaults to 'gaia_dr3_source'."
            ),
        ),
        columns: Optional[List[str]] = typer.Option(
            ...
            "--col",
            help=builtins._(
                "Specific columns to retrieve (comma separated or multiple use). Use 'all' for all columns."
            ),
        ),
        column_filters: Optional[List[str]] = typer.Option(
            ...
            "--filter",
            help=builtins._(
                "Column filters (e.g., 'w1mpro>10', 'ph_qual=A'). Can be specified multiple times."
            ),
        ),
        ...
        try:
            ...
            if coord_obj:
                ...
                if not final_catalog:
                    ...
                    console.print(
                        _(
                            "[yellow]No catalog specified with coordinates. Defaulting to 'gaia_dr3_source' catalog.[/yellow]"
                        )
                    )
                if not final_radius:
                    console.print(
                        _(
                            "[red]Error: Radius is required when coordinates are provided.[/red]"
                        )
                    )
                    raise typer.Exit(code=1)
                console.print(
                    _(
                        "[cyan]Querying IRSA catalog '{catalog}' via Gator for region: '{coordinates}' with radius '{radius}'...[/cyan]"
                    ).format(
                ...
                if final_catalog and final_catalog != target_input:
                    console.print(
                        _(
                            "[red]Error: Catalog name provided as both positional argument and --catalog option. Please use only one.[/red]"
                        )
                    )
                    raise typer.Exit(code=1)
                ...
                if final_radius:
                    console.print(
                        _(
                            "[red]Error: Catalog name '{target_input}' provided as first argument, but a radius '{radius}' was also provided. Catalog queries do not use radius. Please use 'irsa gator <catalog_name>' without a radius, or provide coordinates and a radius.[/red]"
                        ).format(target_input=target_input, radius=radius)
                    )
                    raise typer.Exit(code=1)
                console.print(
                    _(
                        "[cyan]Browsing IRSA catalog '{catalog}' (first {limit} rows)...[/cyan]"
                    ).format(catalog=final_catalog, limit=Irsa.ROW_LIMIT)
                )
                ...
                result_table: Optional[AstropyTable] = Irsa.query_region(
                    coordinates=coord, radius=rad_quantity, catalog=final_catalog
                )
            ...
            if result_table and len(result_table) > 0:
                console.print(
                    _("[green]Found {count} match(es) in '{catalog}'.[/green]").format(
                        count=len(result_table), catalog=final_catalog
                    )
                )
                display_table(
                    ctx,
                    result_table,
                    title=_("IRSA Gator: {catalog}").format(catalog=final_catalog),
                ...
                if output_file:
                    save_table_to_file(
                        ...
                        output_file,
                        output_format,
                        _("IRSA Gator {catalog} query").format(catalog=final_catalog),
                    )
            else:
                console.print(
                    _(
                        "[yellow]No information found in '{catalog}' for the specified region.[/yellow]"
                    ).format(catalog=final_catalog)
                )
        except Exception as e:
            handle_astroquery_exception(
                ctx,
                e,
                _("IRSA Gator query for catalog {catalog}").format(
                    catalog=final_catalog if final_catalog else "unknown"
                ),
            )
            raise typer.Exit(code=1)
    ...
    @app.command(
        name="region",
        help=builtins._("Perform a cone search across multiple IRSA collections."),
    )
    @global_keyboard_interrupt_handler
    def query_region(
        ctx: typer.Context,
        coordinates: str = typer.Argument(
            ..., help=builtins._("Coordinates (e.g., '10.68h +41.26d', 'M31').")
        ),
        radius: str = typer.Argument(
            ..., help=builtins._("Search radius (e.g., '10arcsec', '0.5deg').")
        ),
        collection: Optional[str] = typer.Option(
            None,
            help=builtins._(
                "Specify a collection (e.g., 'allwise', '2MASS'). Leave blank for a general search."
            ),
        ),
        columns: Optional[List[str]] = typer.Option(
            None,
        ...
        console.print(
            _(
                "[cyan]Performing IRSA cone search for region: '{coordinates}' with radius '{radius}'...[/cyan]"
            ).format(coordinates=coordinates, radius=radius)
        )
        try:
            ...
            result_table: Optional[AstropyTable] = Irsa.query_region(
                coordinates=coord, radius=rad_quantity, collection=collection
            )
            ...
            if result_table and len(result_table) > 0:
                console.print(
                    _(
                        "[green]Found {count} match(es) in IRSA holdings.[/green]"
                    ).format(count=len(result_table))
                )
                display_table(
                    ctx,
                    result_table,
                    title=_("IRSA Cone Search Results"),
                    max_rows=max_rows_display,
                    show_all_columns=show_all_columns,
                )
                if output_file:
                    save_table_to_file(
                        ctx, result_table, output_format, _("IRSA cone search query")
                    )
            else:
                console.print(
                    _(
                        "[yellow]No information found in IRSA for the specified region{collection_info}.[/yellow]"
                    ).format(
                        collection_info=_(" in collection {collection}").format(
    ...
    @dust_app.command(
        name="extinction",
        help=builtins._(
            "Get E(B-V) dust extinction values for one or more coordinates."
        ),
    )
    @global_keyboard_interrupt_handler
    def get_extinction(
        ...
        targets: List[str] = typer.Argument(
            ...,
            help=builtins._(
                "Object name(s) or coordinate(s) (e.g., 'M31', '10.68h +41.26d', '160.32 41.45'). Can be specified multiple times."
            ),
        ),
        map_name: str = typer.Option(
            "SFD",
            help=builtins._(
                "Dust map to query ('SFD', 'Planck', 'IRIS'). SFD is Schlegel, Finkbeiner & Davis (1998)."
            ),
        ),
        output_file: Optional[str] = common_output_options["output_file"],
        output_format: Optional[str] = common_output_options["output_format"],
        ...
        console.print(
            _(
                "[cyan]Querying IRSA Dust ({map_name}) for extinction at: {targets_str}...[/cyan]"
            ).format(map_name=map_name, targets_str=", ".join(targets))
        )

        coordinates_list = []
        for target_str in targets:
            try:
                coord = parse_coordinates(ctx, target_str)
                if coord is None:
                    console.print(
                        _(
        ...
        try:
            if len(coordinates_list) == 1:
                table_result = IrsaDust.get_extinction_table(
                    coordinates_list[0], map_name=map_name
                )
            else:
                results = []
                ...
                for i, coord in enumerate(coordinates_list):
                    debug(
                        _(
                            "  Processing target {current_num}/{total_num}: {target_name}"
                        ).format(
                            current_num=i + 1,
                            total_num=len(coordinates_list),
                ...
                if not results:
                    console.print(
                        _(
                            "[yellow]No extinction data retrieved for any target.[/yellow]"
                        )
                    )
                    raise typer.Exit()
            ...
            if table_result is not None and len(table_result) > 0:
                display_table(
                    ctx,
                    table_result,
                    title=_("IRSA Dust Extinction ({map_name})").format(
                        map_name=map_name
                    ),
                )
                if output_file:
                    save_table_to_file(
                        ...
                        output_file,
                        output_format,
                        _("IRSA Dust {map_name} extinction").format(map_name=map_name),
                    )
            else:
                console.print(
                    _(
                        "[yellow]No extinction data returned by IRSA Dust ({map_name}).[/yellow]"
            ...
            handle_astroquery_exception(
                ctx,
                e,
                _("IRSA Dust ({map_name}) get_extinction_table").format(
                    map_name=map_name
                ),
            )
            raise typer.Exit(code=1)
    ...
    @dust_app.command(
        name="map", help=builtins._("Get a FITS image of a dust map for a region.")
    )
    @global_keyboard_interrupt_handler
    def get_map(
        ctx: typer.Context,
        target: str = typer.Argument(
            ...,
            help=builtins._(
                "Central object name or coordinates (e.g., 'M31', '10.68h +41.26d')."
            ),
        ),
        radius: str = typer.Option(
            "1 degree",
            help=builtins._("Radius of the image (e.g., '30arcmin', '1.5deg')."),
        ),
        map_name: str = typer.Option(
            "SFD", help=builtins._("Dust map to query ('SFD', 'Planck', 'IRIS').")
        ),
        output_dir: str = typer.Option(
            ".", "--out-dir", help=builtins._("Directory to save the FITS image(s).")
        ),
        filename_prefix: str = typer.Option(
            "dust_map", help=builtins._("Prefix for the output FITS filename(s).")
        ),
        test: bool = typer.Option(
            False, "--test", "-t", help=_("Enable test mode and print elapsed time.")
        ),
        ...
        console.print(
            _(
                "[cyan]Querying IRSA Dust ({map_name}) for map around '{target}' with radius {radius}...[/cyan]"
            ).format(map_name=map_name, target=target, radius=radius)
        )
        ...
        if not os.path.exists(output_dir):
            ...
            debug(
                _("Created output directory: {output_dir}").format(
                    output_dir=output_dir
                )
            )

        try:
            image_hdulists = IrsaDust.get_images(
                coords, radius=rad_quantity, map_name=map_name, image_type="ebv"
            )

            if not image_hdulists:
                console.print(
                    _(
                        "[yellow]No map images returned by IRSA Dust ({map_name}) for this region.[/yellow]"
                    ).format(map_name=map_name)
                )
                return
            ...
            for i, hdul in enumerate(image_hdulists):
                ...
                if "FILETYPE" in hdul[0].header:
                    map_type_suffix = (
                        f"_{hdul[0].header['FILETYPE'].lower().replace(' ', '_')}"
                    )
                ...
                filename = os.path.join(
                    output_dir,
                    f"{filename_prefix}_{map_name.lower()}{map_type_suffix}_{coords.ra.deg:.2f}_{coords.dec.deg:.2f}.fits",
                )
                hdul.writeto(filename, overwrite=True)
                console.print(
                    _("[green]Saved dust map: {filename}[/green]").format(
                        filename=filename
                    )
                )
                hdul.close()
            ...
            handle_astroquery_exception(
                ctx, e, _("IRSA Dust ({map_name}) get_images").format(map_name=map_name)
            )
            raise typer.Exit(code=1)
