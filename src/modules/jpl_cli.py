...
def get_app():
    ...
    def jpl_callback(
        ...
        if ctx.invoked_subcommand is None and not any(
            ...
            if commands_match:
                ...
                if (
                    len(filtered_commands_section_lines) > 1
                ):  # Check if there's content beyond just the header/footer
    ...
    @app.command(
        name="horizons",
        help=builtins._(
            "Query ephemerides, orbital elements, or vectors for a target object. (h)"
        ),
    )
    ...
    def query_horizons(
        ...
        target: Optional[str] = typer.Argument(
            None,
            help=builtins._(
                "Object ID (e.g., 'Mars', 'Ceres', '2000NM', '@10'). Use '@' prefix for spacecraft ID."
            ),
        ),
        ...
        start_time: Optional[str] = typer.Option(
            ...
            "--start",
            help=builtins._(
                "Start time for ephemeris range (YYYY-MM-DD [HH:MM]). Overrides 'epochs' if 'end_time' is also set."
            ),
        ),
        end_time: Optional[str] = typer.Option(
            None,
            "--end",
            help=builtins._("End time for ephemeris range (YYYY-MM-DD [HH:MM])."),
        ),
        step: Optional[str] = typer.Option(
            "1d",
            "--step",
            help=builtins._(
                "Time step for ephemeris range (e.g., '1d', '1h', '10m'). Used if 'start_time' and 'end_time' are set."
            ),
        ),
        location: str = typer.Option(
            "500",
            help=builtins._(
                "Observatory code (e.g., '500' for Geocenter, 'geo' is alias for '500'). Try common codes or find specific ones."
            ),
        ...
        id_type: Optional[IDType] = typer.Option(
            ...
            help=builtins._(
                "Type of the target identifier. If None, Horizons will try to guess."
            ),
        ),
        ...
        quantities: Optional[str] = typer.Option(
            None,
            help=builtins._(
                "Comma-separated string of quantity codes (e.g., '1,2,19,20'). Relevant for OBSERVER and VECTORS. See JPL Horizons docs for codes. Uses sensible defaults if None."
            ),
        ),
        ...
        show_all_columns: bool = typer.Option(
            False,
            "--show-all-cols",
            help=builtins._("Show all columns in the output table, even if wide."),
        ),
        jpl_server: str = typer.Option(
            "nasa",
            help=builtins._(
                "JPL Horizons server to use. Choices: {server_list}"
            ).format(server_list=list(JPL_SERVERS.keys())),
        ...
        console.print(
            _("[cyan]Querying JPL Horizons for '{target}'...[/cyan]").format(
                target=em(str(target))
            )
        )
        ...
        if start_time and end_time:
            ...
            debug(
                _("Using epoch range: {start} to {end} with step {step}").format(
            ...
            if epochs.startswith("{") and epochs.endswith("}"):
                try:
                    ...
                    debug(
                        _("Using epoch dict: {epoch_dict}").format(
                    ...
                    console.print(
                        _(
                            "[red]Error parsing epoch dictionary: {error}. Please ensure it's a valid Python dictionary string.[/red]"
                        ).format(error=em(str(e)))
                    )
        ...
        auto_id_type = (
            id_type.value
            if id_type
            else ("majorbody" if target.strip().lower() in auto_majorbodies else None)
        )
        query_params = {
            "id": target,
            "location": location,
        ...
        try:
            ...
            table_title = _("{ephem_type} for {target}").format(
                ephem_type=ephem_type.value, target=em(str(target))
            )
            result_table = None
            ...
            if ephem_type == EphemType.OBSERVER:
                ...
                if q:
                    debug(
                        _("Requesting quantities for vectors: {quantities}").format(
                            quantities=q
                        )
                    )
                result_table = (
                    obj.vectors(quantities=q, get_raw_response=False)
                    if q
                    else obj.vectors(get_raw_response=False)
                )
                ...
                try:
                    ...
                    display_table(
                        ctx,
                        eph_table,
                        title="Ephemerides for today",
                        max_rows=max_rows,
                        show_all_columns=show_all_columns,
                    )
                except Exception as e:
                    console.print(f"[red]Ephemerides table error: {em(str(e))}[/red]")
            ...
            if result_table is not None and len(result_table) > 0:
                ...
                console.print(
                    _(
                        "[yellow]No results found for '{target}' with the specified parameters.[/yellow]"
                    ).format(target=em(str(target)))
                )
    ...
    def query_sbdb(
        ctx: typer.Context,
        target: Optional[str] = typer.Argument(
            None, help=builtins._("Target small body (e.g., 'Ceres', '1P', '2023 BU').")
        ),  # Make target optional for help display
        id_type: Optional[str] = typer.Option(
            None,
            help=builtins._(
                "Type of target identifier ('name', 'des', 'moid', 'spk') (default: let SBDB auto-detect)."
            ),
        ),
        phys_par: bool = typer.Option(
            False, "--phys-par", help=builtins._("Include physical parameters.")
        ),
        orb_el: bool = typer.Option(
            False, "--orb-el", help=builtins._("Include orbital elements.")
        ),
        close_approach: bool = typer.Option(
            False, "--ca-data", help=builtins._("Include close-approach data.")
        ),
        radar_obs: bool = typer.Option(
            False, "--radar-obs", help=builtins._("Include radar observation data.")
        ),
        ...
        max_rows_display: int = typer.Option(
            20,
            help=builtins._(
                "Maximum number of rows to display for tables. Use -1 for all rows."
            ),
        ),
        show_all_columns: bool = typer.Option(
            False,
            "--show-all-cols",
            help=builtins._("Show all columns in output tables."),
        ),
        test: bool = typer.Option(
            False, "--test", "-t", help=_("Enable test mode and print elapsed time.")
        ),
        ...
        console.print(
            _("[cyan]Querying JPL SBDB for target: '{target}'...[/cyan]").format(
                target=em(str(target))
            )
        )
        try:
            ...
            sbdb_query = SBDB.query(target, **query_kwargs, full_precision=True)

            if sbdb_query:
                console.print(
                    _("[green]Data found for '{target}'.[/green]").format(target=em(str(target)))
                )
                if isinstance(sbdb_query, AstropyTable) and len(sbdb_query) > 0:
                    display_table(
                        ctx,
                        sbdb_query,
                        title=_("JPL SBDB Data for {target}").format(target=em(str(target))),
                    ...
                    if output_file:
                        save_table_to_file(
                            ...
                            output_file,
                            output_format,
                            _("JPL SBDB query for {target}").format(target=em(str(target))),
                        )

                elif hasattr(sbdb_query, "items"):
                    object_fullname = sbdb_query.get("object", {}).get(
                        "fullname", target
                    )
                    console.print(
                        _(
                            "[bold magenta]SBDB Data for: {fullname}[/bold magenta]"
                        ).format(fullname=object_fullname)
                    )
                    output_data = {}
                    ...
                    def process_quantity_objects(obj):
                        if isinstance(obj, dict):
                            return {
                                k: process_quantity_objects(v) for k, v in obj.items()
                            }
                    ...
                    for key, value in sbdb_query.items():
                        ...
                        if key == "object":
                            ...
                            if isinstance(processed_value, dict):
                                ...
                                display_table(
                                    ctx, [[str(processed_value)]], title="Object"
                                )
                                output_data[str(key)] = processed_value
                        elif key == "orbit":
                            field_order = [
                            ...
                            if isinstance(processed_value, dict):
                                ...
                                display_table(
                                    ctx, [[str(processed_value)]], title="Orbit"
                                )
                                output_data[str(key)] = processed_value
                        elif key == "model_pars":
                            field_order = [
                            ...
                            if isinstance(processed_value, dict):
                                ...
                                display_table(
                                    ctx, [[str(processed_value)]], title="Elements"
                                )
                                output_data[str(key)] = processed_value
                    ...
                    if output_file and not any(
                        isinstance(v, AstropyTable) for v in sbdb_query.values()
                    ):
                        import json
                        ...
                        try:
                            ...
                            console.print(
                                _(
                                    "[green]Primary data saved to {file_path}[/green]"
                                ).format(file_path=file_path)
                            )
                        except Exception as json_e:
                            console.print(
                                _(
                                    "[red]Could not save non-table data as JSON: {error}[/red]"
                                ).format(error=em(str(json_e)))
                            )
                else:
                ...
                console.print(
                    _(
                        "[yellow]No information found for target '{target}'.[/yellow]"
                    ).format(target=em(str(target)))
                )
