...
def run_field():
    ...
    # Mapping of src.modules to astroquery modules
    # Add more mappings here if the module name in astroquery is different from src.modules
    MODULE_MAP = {
        "alma": "alma",
        "esasky": "esasky",
        "gaia": "gaia",
    ...
    for filename in os.listdir(module_dir):
        if filename.endswith("_cli.py") and filename != "__init__.py":
            ...
            try:
                # Dynamically import src module
                cli_module = importlib.import_module(f"src.modules.{module_name}_cli")

                # Dynamically import astroquery module
                astroquery_module = importlib.import_module(
                    f"astroquery.{astroquery_module_name}"
                )
                ...
                official_fields = set()
                local_fields = set(
                    getattr(cli_module, f"{module_name.upper()}_FIELDS", [])
                )
                ...
                try:
                    if module_name == "simbad":
                        official_fields = set(
                            str(row[0])
                            for row in astroquery_module.Simbad.list_votable_fields()
                        )
                        ...
                        try:
                            ...
                            if results is not None:
                                ...
                                print(
                                    f"ALMA query returned no results, skipping field check."
                                )
                                continue
                        ...
                        print(
                            f"Note: Automatic field retrieval for MAST is not directly supported by a simple method like 'list_fields'."
                        )
                        print(
                            f"Please refer to MAST documentation for available query parameters and result columns."
                        )
                        continue  # Skip field check for MAST
                    elif module_name == "vizier":
                        print(
                            f"Note: VizieR fields are catalog-specific. Please specify a catalog to view its fields."
                        )
                        print(
                            f"Example: `aqc vizier object M31 5arcmin --catalog I/261/gaiadr3 --col all`"
                        )
                        ...
                        try:
                            ...
                            if tables:
                                ...
                                print(
                                    f"Note: Gaia fields are extensive and table-specific (TAP service)."
                                )
                                print(
                                    f"Please refer to Gaia TAP documentation for specific table columns."
                                )
                                continue
                            else:
                                print(
                                    f"Gaia.load_tables() returned no tables, skipping field check."
                                )
                                continue
                        except Exception as e:
                        ...
                        try:
                            ...
                            if tables:
                                ...
                                columns = astroquery_module.Heasarc.get_columns(
                                    first_table
                                )
                                official_fields = set(col.name for col in columns)
                            else:
                                print(
                                    f"Heasarc.get_tables() returned no tables, skipping field check."
                                )
                                continue
                        except Exception as e:
                        ...
                        try:
                            official_fields = set(
                                astroquery_module.SDSS.get_available_columns()
                            )
                        except Exception as e:
                        ...
                        try:
                            ...
                            results = astroquery_module.NIST.get_transitions(
                                wavelength_range=(1000, 1001), energy_unit="eV", top=1
                            )
                            if results is not None and len(results) > 0:
                                official_fields = set(results.colnames)
                            else:
                                print(
                                    f"NIST query returned no results, skipping field check."
                                )
                                continue
                        except Exception as e:
                        ...
                        for attr_name in [
                            ...
                            if hasattr(astroquery_module, attr_name):
                                try:
                                    ...
                                    print(
                                        f"Attempt with {attr_name} failed for {module_name.upper()}: {e}"
                                    )
                        ...
                        if not found_fields:
                            ...
                            try:
                                if hasattr(astroquery_module, "query_object"):
                                    ...
                                    results = astroquery_module.query_object(
                                        "M31", radius="0.01 deg", maxrec=1
                                    )
                                    if results is not None and len(results) > 0:
                                        official_fields = set(results.colnames)
                                        found_fields = True
                                    ...
                                    results = astroquery_module.query_region(
                                        "M31", radius="0.01 deg", maxrec=1
                                    )
                                    if results is not None and len(results) > 0:
                                        official_fields = set(results.colnames)
                                ...
                                print(
                                    f"Attempt with generic query for {module_name.upper()} failed: {e}"
                                )
                                pass  # Continue to next method if one fails
                        ...
                        if not found_fields:
                            if module_name in ["jplhorizons", "jplsbdb"]:
                                print(
                                    f"Note: {module_name.upper()} fields are dynamic and depend on the specific query. Automatic field retrieval is not applicable."
                                )
                            else:
                                print(
                                    f"Could not determine how to get official fields for {module_name.upper()} using common methods. Please refer to its documentation. Skipping."
                                )
                            continue

                except Exception as e:
                    print(
                        f"Error getting official fields for {module_name.upper()}: {e}"
                    )
                ...
                if extra:
                    print(
                        f"{module_name.upper()}_FIELDS contains invalid fields: {extra}"
                    )
